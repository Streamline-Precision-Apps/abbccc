...
import {initialUserSettings, intialEquipment, initialUsers, initialContacts, 
  initialJobsites, initialTimeSheets, initialCrews, initialCrewMembers,
  initialCostCodes,initialCrewJobsites } from "@/data/dataValues";
  ...
  try {
    ...
    for (const user of initialUsers) {
      try {
        ...
        const newUser = await prisma.users.create({ data: { ...user, password: hashed } });
    ...
    for (const timesheet of initialTimeSheets) {
      try {
        const newTimeSheet = await prisma.timeSheets.create({ data: timesheet });
    ...
    for (const crewjobsite of initialCrewJobsites) {
      try {
        const newCrewJobsite = await prisma.crewJobsites.create({ data: crewjobsite });
    ...
    for (const crewMember of initialCrewMembers) {
      try {
        const newCrewMember = await prisma.crewMembers.create({ data: crewMember });
        console.log("Created crew member with crewId: ", newCrewMember.crewId, " and employeeId: ", newCrewMember.employeeId);
      } catch (error) {
