...
export async function adminUpdateTimesheet(
  ...
  if (
    !data.form.jobsite ||
    !data.form.jobsite.id ||
    !data.form.costcode ||
    !data.form.costcode.name
  ) {
    throw new Error(
      "Missing required user, jobsite, or costcode in timesheet update payload"
    );
  }

  // Update the main timesheet and all related logs in a transaction
  console.log("Updating timesheet with id/data:", id, data);
  await prisma.$transaction(async (tx) => {
    ...
    // Delete all related logs for this timesheet
    await tx.maintenanceLog.deleteMany({ where: { timeSheetId: id } });
    const truckingLogs = await tx.truckingLog.findMany({
      where: { timeSheetId: id },
    });
    for (const tlog of truckingLogs) {
      await tx.equipmentHauled.deleteMany({
        where: { truckingLogId: tlog.id },
      });
      await tx.material.deleteMany({ where: { truckingLogId: tlog.id } });
      await tx.refuelLog.deleteMany({ where: { truckingLogId: tlog.id } });
      await tx.stateMileage.deleteMany({ where: { truckingLogId: tlog.id } });
    }
    await tx.truckingLog.deleteMany({ where: { timeSheetId: id } });
    const tascoLogs = await tx.tascoLog.findMany({
      where: { timeSheetId: id },
    });
    for (const tasco of tascoLogs) {
      await tx.refuelLog.deleteMany({ where: { tascoLogId: tasco.id } });
    }
    await tx.tascoLog.deleteMany({ where: { timeSheetId: id } });
    await tx.employeeEquipmentLog.deleteMany({ where: { timeSheetId: id } });

    // Recreate logs from new data (same as create)
    // Maintenance Logs
    for (const log of data.maintenanceLogs) {
      if (!log.maintenanceId) continue;
      await tx.maintenanceLog.create({
