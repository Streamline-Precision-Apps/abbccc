 ...
 */
const HorizontalScrollbar: React.FC<HorizontalScrollbarProps> = ({ containerRef }) => {
  ...
  const updateThumb = () => {
    ...
    setThumbWidth((clientWidth / scrollWidth) * (trackRef.current?.clientWidth || 1));
  };
  ...
  const onDrag = (e: MouseEvent) => {
    ...
    const ratio = Math.max(0, Math.min(1, (x - thumbWidth / 2) / (trackRect.width - thumbWidth)));
  ...
  const thumbStyle: React.CSSProperties = {
    ...
    transition: 'width 0.25s cubic-bezier(.4,1.6,.4,1), height 0.25s cubic-bezier(.4,1.6,.4,1), box-shadow 0.25s, background 0.2s',
  ...
  return (
    <div style={{ width: '100%', height: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none', position: 'relative', padding: '0 5%' }}>
