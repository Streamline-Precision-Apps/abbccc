...
export const InboxContent = ({ employees, setFilter }: Props) => {
  ...
  const selectEmployee = (employee: TimeRequest) => {
    setTerm(""); // Clear search term when selecting an employee
    router.push(`/admins/inbox/${employee.id}`); // Navigate to the employee's detail page
  };

  const createEmployee = () => {
    router.push(`/admins/inbox/new-request`); // Navigate to the new request page
  };
